  ...
  return (
    <div>
      ...
      >
        {data.map((object, i) => (
          <Paper elevation={20} onClick={() => handleOpen(object)}>
            <Box className='pad25 elipseit content justifyme' ><p dangerouslySetInnerHTML={{__html: getEliptedText(object)}}/></Box>
          </Paper>
        ))}
      </Box>
      ...
      >
        <Fade in={open}>
          <Box sx={style}>
            <p className="content justifyme" dangerouslySetInnerHTML={{__html: text}} />
          </Box>
        </Fade>
      </Modal>
    </div>
